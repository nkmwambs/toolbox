...
if(is_numeric($param3)){
	...
	$this->session->set_userdata('fcp_id',$param3);
}

$fcp_id = $this->session->fcp_id;
...
if($param3>0){
	...
	$account = $this->db->get_where('accounts',array('AccNo'=>$rev_acc_no))->row();

	$account_name = $account->AccName;

	$accNo = $account->AccText;

	$parentAccID = $account->accID;
				...
				<table class="table table-hover" id="varTable">
							...
							<tbody>
								<?php
									...
									$per_var = $this->finance_model->months_budget_variance_percent_per_expense_account($fcp_id,get_fy($param2,$fcp_id),$row->AccNo,$param2);//$this->finance_model->percentage_budget_variance_per_account($param2,$row['AccNo']);
									$var = $this->finance_model->months_budget_variance_per_expense_account($fcp_id,get_fy($param2,$fcp_id),$row->AccNo,$param2);//$this->finance_model->budget_variance_per_account($param2,$row['AccNo']);
								...
								?>
										...
										<tr>
											...
											<input type="hidden" value="<?php echo $fcp_id;?>" name="rec[<?= $counter?>][icpNo]"/>
											...
											<td>
												<div class="form-group">
													<?php
														...
														if($this->db->get_where('varjustify',array('icpNo'=>$fcp_id,'reportMonth'=>$param2,'AccNo'=>$row->AccNo))->num_rows()>0){
															$var_statement = $this->db->get_where('varjustify',array('icpNo'=>$fcp_id,'reportMonth'=>$param2,'AccNo'=>$row->AccNo))->row()->Details;
														}
