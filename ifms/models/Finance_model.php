...
class Finance_model extends CI_Model {
	...
	function project_system_start_date($param1 = "") {
		...
		return $start_date;
		//date("Y-m-01",strtotime('first day of next month',strtotime($this->db->get_where('opfundsbalheader',array('icpNo'=>$param1,"systemOpening"=>'1'))->row()->closureDate)));
	}
	...
	public function statement_balance($project_id = '', $date = "") {
		...
		return $statement;
		//$this->db->get_where('statementbal',array('month'=>$param1))->row();
	}
	...
	public function budgeted_revenue_accounts($as_object = true) {
		...
		if ($as_object) {
			...
			$rev = $this -> db -> order_by('AccNo') -> get_where('accounts', array('AccGrp' => '1', 'budget' => '1', 'active' => '1')) -> result_array();
		}
	...
	function expense_accounts_grouped_by_income() {
		...
		$array_of_income = group_array_by_key($this -> finance_model -> budgeted_revenue_accounts(false), 'accID', $remove_columns);

		foreach ($array_of_income as $key => $value) {
			$grouped_expenses[$key] = $value;
			$grouped_expenses[$key][1] = group_array_by_key($this -> finance_model -> expense_accounts($key, false), 'AccNo', $remove_columns);
		}
	...
	public function months_in_year($date, $show_year = false) {
		...
		if (!$show_year) {
			foreach ($month_num_range as $num) {
				$add_num = $num + 1;
				if ($num == 0)
					$months['month_' . $add_num . '_amount'] = date('M', strtotime($start_month));
				else
					$months['month_' . $add_num . '_amount'] = date('M', strtotime('+' . $num . ' month', strtotime($start_month)));
			}
		} else {
			foreach ($month_num_range as $num) {
				$add_num = $num + 1;
				if ($num == 0)
					$months['month_' . $add_num . '_amount'] = date('M Y', strtotime($start_month));
				else
					$months['month_' . $add_num . '_amount'] = date('M Y', strtotime('+' . $num . ' month', strtotime($start_month)));
			}
